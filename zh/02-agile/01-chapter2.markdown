...
产品待办事项列表是按照优先级排列的要完成的工作的概要需求，在团队的**计划会议（Planning meeting）**中，开发团队决定在下一次Sprint中他们能够承诺完成多少功能，这就形成了**Sprint待办事项列表（Sprint backlog)**。
...
评审会议后的**回顾会议（Retrospective meeting）**是帮助团队发现问题，并提出行之有效的方式进行提高。
